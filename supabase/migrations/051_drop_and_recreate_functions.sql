...
-- 1. Drop all existing functions that might have conflicts
...
-- 2. Recreate add_card_to_deck_v3 with correct column names
...
BEGIN
  ...
  -- Determine max copies allowed in deck (NEW RULES)
...
-- 3. Recreate unlock_all_cards_for_user function
CREATE FUNCTION unlock_all_cards_for_user(user_uuid UUID)
RETURNS JSON AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    -- Determine collection quantity based on new rules
...
-- 4. Recreate starter collection function
CREATE FUNCTION create_starter_collection_for_user(user_uuid UUID)
RETURNS VOID AS $$
DECLARE
  card_record RECORD;
  collection_quantity INTEGER;
...
-- 5. Create deck validation function (fresh creation)
CREATE FUNCTION validate_deck(deck_uuid UUID)
RETURNS JSON AS $$
DECLARE
  deck_owner UUID;
  total_cards INTEGER;
...
-- 6. Update existing collections to match new rules
DO $$
DECLARE
...
DO $$ 
BEGIN
  RAISE NOTICE '=== FUNCTIONS RECREATED WITH NEW COLLECTION RULES ===';
  RAISE NOTICE 'All functions dropped and recreated to avoid return type conflicts';
  RAISE NOTICE 'Collection rules now active: Exotic(1), Legendary(2), Regular(4)';
  RAISE NOTICE 'Deck rules now active: Exotic(1), Legendary(2), Regular(2), Size(30)';
END $$;
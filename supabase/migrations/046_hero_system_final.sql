-- FINAL Hero System Setup - Potato King Only
-- Handles all dependencies properly

-- Step 1: Drop all triggers first (they depend on functions)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Step 2: Drop all functions (now safe since triggers are gone)
DROP FUNCTION IF EXISTS get_user_active_hero(uuid) CASCADE;
DROP FUNCTION IF EXISTS give_potato_king_to_user(uuid) CASCADE;
DROP FUNCTION IF EXISTS update_hero_battle_stats(uuid, text, boolean) CASCADE;
DROP FUNCTION IF EXISTS unlock_starter_heroes_for_user(uuid) CASCADE;
...
-- Step 4: Create heroes table (simplified)
CREATE TABLE heroes (
...
-- Step 5: Create user_heroes table (tracks user's hero stats)
...
-- Step 9: Insert Potato King hero data
...
-- Step 10: Create functions (now safe to create)
CREATE FUNCTION give_potato_king_to_user(user_uuid UUID)
RETURNS VOID AS $$
BEGIN
...
CREATE FUNCTION update_hero_battle_stats(user_uuid UUID, hero_id_param TEXT, won BOOLEAN)
RETURNS VOID AS $$
BEGIN
  IF won THEN
    UPDATE user_heroes 
...
CREATE FUNCTION get_user_active_hero(user_uuid UUID)
RETURNS TABLE(
  hero_id TEXT,
  name TEXT,
  description TEXT,
...
-- Step 11: Give Potato King to all existing users
INSERT INTO user_heroes (user_id, hero_id, is_active)
...
-- Step 12: Create trigger (now that function exists)
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION handle_new_user_signup();

-- Step 13: Verify setup
SELECT 'Setup Complete! Heroes:' as info, count(*) as count FROM heroes;
SELECT 'User Heroes Created:' as info, count(*) as count FROM user_heroes;

-- Server-side Matchmaking System (Safe Version)
-- This migration safely creates matchmaking logging and queue management without conflicts

-- Create a logs table for matchmaking events (if not exists)
...
-- Enable RLS for logs table (safe to run multiple times)
ALTER TABLE public.matchmaking_logs ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist, then recreate them
DROP POLICY IF EXISTS "Allow system to insert logs" ON public.matchmaking_logs;
DROP POLICY IF EXISTS "Allow authenticated users to read logs" ON public.matchmaking_logs;
...
-- Create or replace the logging function
...
-- Safely create the trigger (drop first if exists)
...
-- Success message
SELECT 'Server-side matchmaking system created successfully! (Safe Version - No Conflicts)' as status;
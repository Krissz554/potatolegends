-- Fix Real-time Battle Notifications (Safe Version)
...
-- Try to add to realtime publication, but handle if already exists
DO $$
BEGIN
    -- Add matchmaking_queue to realtime if not already there
    BEGIN
    ...
    -- Add battle_sessions to realtime if not already there
    BEGIN
    ...
    -- Add battle_actions to realtime if not already there
    BEGIN
...
-- Success message
SELECT 'Real-time battle notifications fixed! (Safe Version - No Conflicts)' as status;
...
namespace PotatoCardGame.Network
{
    ...
    public class RealSupabaseClient : MonoBehaviour
    {
        ...
        #region Deck System (Real Implementation)
        
        public async Task<List<Deck>> LoadUserDecks()
        {
            try
            {
                var decks = await GetData<List<Deck>>($"decks?user_id=eq.{userId}&select=*&order=created_at.desc");
                
                if (decks != null)
                {
                    Debug.Log($"🔧 Loaded {decks.Count} user decks");
                    return decks;
                }
